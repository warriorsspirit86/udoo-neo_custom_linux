...
#define AACD_MAX_NUM_MEM_REQS   5
...
#define AACD_INPUT_BUFFER_SIZE  2048
...
typedef enum
{
    ...
    /* The output data can't be used by caller */
    AACD_ERROR_INIT = 51,
...
typedef struct AACD_decoder_config_struct
{
    ...
    AACD_INT8           (*app_swap_buf) (AACD_UCHAR **new_buf_ptr,
   ...
   void (*dump_bsac_data)(int index, int* pbuf, int length);
#endif
} AACD_Decoder_Config; 
...
/* Function prototypes */
 ...
 EXTERN AACD_RET_TYPE aacd_decode_frame (AACD_Decoder_Config *dec_config,
     				 ...
     				 AACD_OutputFmtType out_buf[][AAC_FRAME_SIZE],
				 AACD_INT8* buf, AACD_INT32 buf_len);
EXTERN AACD_INT8 app_swap_buffers_aac_dec (AACD_UCHAR **new_buf_ptr,
                               AACD_UINT32 *new_buf_len,
                               void* global_struct); /* TLSbo84529 */

//DSPhl28187
#ifdef ENABLE_DEBUG_LOG
void aacd_register_debug_funcptr(int (*text_debug_ptr)(short int msgid,char *fmt,...), \
